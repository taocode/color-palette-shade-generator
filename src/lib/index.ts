...
export const placeholder = (i, phColor) => {
  const hName = hueName(parseToHsla(phColor)[0])
  const scheme = get(schemeObj)
...
export const schemeHues = () => {
  const hues = get(schemeObj).hues
  const primaryHue = get(hue)
